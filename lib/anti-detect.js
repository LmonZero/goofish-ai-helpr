...
module.exports = function antiDetectScript() {
    // ===== User-Agent 修复：HeadlessChrome → Chrome =====
    // patchright 在 headless 模式下默认 UA 包含 "HeadlessChrome"
    // 这是最容易被闲鱼等网站检测的标识
    const _realUA = navigator.userAgent;
    const _fixedUA = _realUA.replace('HeadlessChrome/', 'Chrome/');
    if (_realUA !== _fixedUA) {
        Object.defineProperty(navigator, 'userAgent', {
            get: () => _fixedUA,
            configurable: true,
        });
        // appVersion 也含 HeadlessChrome，必须同步修复
        const _realAppVersion = navigator.appVersion;
        const _fixedAppVersion = _realAppVersion.replace('HeadlessChrome/', 'Chrome/');
        Object.defineProperty(navigator, 'appVersion', {
            get: () => _fixedAppVersion,
            configurable: true,
        });
    }

    // ===== navigator.webdriver 消除（兜底） =====
    // patchright 底层已处理，这里加兜底确保万无一失
    ...
    // ===== Chrome 运行时伪装 =====
    // 真实 Chrome 的 window.chrome 有完整的 runtime/app/csi/loadTimes 等
    if (!window.chrome) {
        window.chrome = {};
    }
    if (!window.chrome.runtime) {
        // 真实 Chrome runtime 包含 id, onMessage, onConnect 等
    ...
    // csi 和 loadTimes 是旧版 Chrome API，但某些检测脚本会检查
    ...
    if (!window.chrome.loadTimes) {
        window.chrome.loadTimes = function () {
            return {
                ...
                firstPaintTime: performance.timing?.responseEnd / 1000 || 0,
    ...
    // 以下函数也被伪装，必须标记
    if (navigator.permissions?.query && navigator.permissions.query !== originalQuery) {
        _markPatched(navigator.permissions.query);
    }
    if (navigator.getBattery) {
        _markPatched(navigator.getBattery);
    }
    // Chrome runtime/app 伪装函数
    if (window.chrome?.csi) _markPatched(window.chrome.csi);
    if (window.chrome?.loadTimes) _markPatched(window.chrome.loadTimes);
};
